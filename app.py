...
.kpi-card .label { font-size: 0.78rem; color: #666; margin-top: 4px; }
...
.section-title {
    ...
    font-size:1.05rem; font-weight:700; margin:15px 0 10px;
}
.header-banner {
...
.header-banner h1 { font-size:1.7rem; margin:0; color:white; }
.header-banner p  { font-size:0.88rem; opacity:.85; margin:5px 0 0; color:white; }
...
# LOGIN
# ══════════════════════════════════════════════════════════════════════════════
USERS = {"admin": "souss2025", "مدير": "planif2025"}
...
if not st.session_state.logged_in:
    st.markdown("""
    <div style="max-width:420px;margin:60px auto;background:white;border-radius:20px;
                padding:40px;box-shadow:0 8px 40px rgba(0,0,0,0.12);text-align:center;">
        <div style="font-size:3.5rem">🏫</div>
        <h2 style="color:#1a3a5c;font-size:1.5rem;margin-bottom:6px;">لوحة القيادة</h2>
        <p style="color:#888;font-size:0.9rem;margin-bottom:20px;">التعليم الابتدائي — جهة سوس ماسة</p>
    </div>
    """, unsafe_allow_html=True)
...
def load_data(file):
    ...
    num_cols = [
        ...
        'مج التلميذات','الاناث بالأقسام المشتركة',
        'مج الأقسام','الأقسام المشتركة منهم'
    ]
    for c in num_cols:
    ...
    etab = df.groupby('CD_ETAB').agg(
        nom_ar=('NOM_ETABA','first'), type_etab=('LA_NETAB','first'),
        prov=('LA_PROV','first'), commune=('COMMUNE','first'),
        milieu=('LA_MIL','first'), cd_rat=('Cd_RAT','first'),
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("ارفع ملف Excel المحدّث", type=["xlsx"])
    ...
    selected_prov = st.selectbox("📍 المديرية", ["الكل"] + PROVINCES)
    ...
    selected_commune = st.selectbox("🏘️ الجماعة", ["الكل"] + communes)

    st.markdown("**🏫 البحث عن مؤسسة**")
    search_text = st.text_input("اكتب الاسم أو الرمز")
    if search_text:
        mask = (etab['nom_ar'].str.contains(search_text,na=False) |
        ...
        for _, row in etab[mask][['CD_ETAB','nom_ar']].head(12).iterrows():
...
for col, cls, val, lbl, icon in [
    (cols[0],"kpi-blue",   total_etab,         "عدد المؤسسات",   "🏫"),
    (cols[1],"kpi-green",  centrales,           "مدارس مركزية",   "🏛️"),
    (cols[2],"kpi-orange", annexes,             "فروع وملاحق",    "🔗"),
    (cols[3],"kpi-purple", f"{total_eleves:,}", "إجمالي التلاميذ","👥"),
    (cols[4],"kpi-red",    f"{total_filles:,}", "منهم الإناث",    "👧"),
    (cols[5],"kpi-teal",   total_classes,       "إجمالي الأقسام", "📚"),
]:
    with col:
        st.markdown(f'<div class="kpi-card {cls}"><div class="value">{icon}<br>{val}</div><div class="label">{lbl}</div></div>', unsafe_allow_html=True)
...
with ca:
    ...
    fig.add_trace(go.Bar(name='فروع وملاحق', x=ps['prov'], y=ps['annexes'],   marker_color='#42a5f5', text=ps['annexes'],   textposition='inside'))
...
fig_map = px.scatter_mapbox(
    ...
    color_discrete_map={'مركزية':'#1a3a5c','فرع/ملحقة':'#42a5f5'},
